      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4">
        <div className="flex items-center justify-between h-14 sm:h-16">
          <div className="flex items-center gap-3">
            <div 
              className="w-6 h-6 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center text-white font-bold text-sm sm:text-base"
              style={{ backgroundColor: COLORS.accent }}
            >
              S
            </div>
            <h1 className="text-lg sm:text-xl font-bold hidden sm:block" style={{ color: COLORS.dark }}>
              SAVIA Gestión
            </h1>
          </div>
          ...
          <div className="flex space-x-1 items-center overflow-x-auto">
            {allowedModules.map(({ id, label, icon: Icon }) => (
              <button
                ...
                className={`
                  flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1 sm:py-2 rounded-lg transition-all duration-200 whitespace-nowrap
                  ${activeModule === id 
            ...
            <button
              onClick={onLogout}
              className="flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1 sm:py-2 rounded-lg text-gray-600 hover:text-gray-900 hover:bg-gray-100 whitespace-nowrap"
            >
